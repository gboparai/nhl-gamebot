import { shouldRetry, getMimeType, generateGameHashtags, validateMediaPaths, teamHashtag, } from "../src/social/utils";
jest.mock("fs", () => ({
    existsSync: jest.fn(),
}));
...
const { logger } = jest.requireMock("../src/logger");
...
describe("src/social/utils", () => {
    ...
    test("validateMediaPaths filters out non-existent files", () => {
        fsMock.existsSync.mockImplementation((path) => path === "valid.png");
