import { createShotChart, createAndSaveShotChart, extractShotEvents, } from "../src/graphic/shotChart";
...
jest.mock("sharp", () => {
    return jest.fn(() => ({
        ...
        toFile: jest.fn().mockImplementation((filePath) => {
...
describe("shotChart", () => {
    ...
    beforeEach(() => {
        jest.clearAllMocks();
        fetchPlayByPlay.mockResolvedValue(mockPlayByPlay);
    });
    afterEach(() => {
        // Clean up test files
    ...
    describe("extractShotEvents", () => {
        it("should extract shot events from play-by-play data", () => {
            const shots = extractShotEvents(mockPlayByPlay);
        ...
        it("should handle missing coordinates gracefully", () => {
            ...
            const shots = extractShotEvents(playByPlayWithMissing);
    ...
    describe("createShotChart", () => {
        ...
        it("should create output directory if it doesn't exist", async () => {
            const nestedPath = path.join(__dirname, "../temp/nested/dir/shot-chart.png");
            try {
        ...
        it("should handle games with no shots gracefully", async () => {
            ...
            fetchPlayByPlay.mockResolvedValue(emptyPlayByPlay);
            await createShotChart(mockGameId, testOutputPath);
            // Should not throw, but also shouldn't create a file
            expect(fs.existsSync(testOutputPath)).toBe(false);
        });
        it("should handle API errors gracefully", async () => {
            fetchPlayByPlay.mockRejectedValue(new Error("API Error"));
            await expect(createShotChart(mockGameId, testOutputPath)).rejects.toThrow("API Error");
        });
    });
