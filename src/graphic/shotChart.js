 ...
 */
function convertCoordinates(xCoord, yCoord, homeDefendingSide, isHomeTeam, normalizeByTeam) {
 ...
 */
export function extractShotEvents(playByPlay, options) {
    const shots = [];
    let skippedGoals = 0;
    for (const play of playByPlay.plays) {
        ...
        const isShotEvent = play.typeDescKey === "shot-on-goal" ||
        ...
        if (options?.homeTeamOnly || options?.awayTeamOnly) {
            ...
            if (options.homeTeamOnly && shootingTeam !== "home")
                continue;
            if (options.awayTeamOnly && shootingTeam !== "away")
                continue;
        }
        ...
        const coords = convertCoordinates(play.details.xCoord, play.details.yCoord, homeDefendingSide, isHomeTeam, options?.normalizeByTeam);
        shots.push({
            coordinates: coords,
 ...
 */
function getShotTeam(play, playByPlay) {
 ...
 */
function getShotType(typeDescKey) {
    switch (typeDescKey) {
        case "goal":
            return "goal";
        case "shot-on-goal":
 ...
 */
export async function createShotChart(gameId, outputPath, options = {}) {
    try {
 ...
 */
export async function createAndSaveShotChart(gameId, options = {}) {
