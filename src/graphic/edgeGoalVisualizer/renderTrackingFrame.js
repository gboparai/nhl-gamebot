import { createRink } from "./createRink";
 ...
 */
export function renderTrackingFrame(rinkParams, trackingFrame, options = {}) {
    const { highlightPlayerId, showNumbers = true, playerRadius = 38, puckRadius = 12, } = options;
    ...
    for (const [key, entity] of Object.entries(trackingFrame.onIce)) {
        ...
        if (isPuck) {
            // Render puck
            puckElements.push(`<g style="transform: translate(${entity.x}px, ${entity.y}px);">` +
 ...
 * @param options - Rendering options
 */
export function renderTrackingFrameToFile(rinkParams, trackingFrame, outputPath, options = {}) {
