...
import { getTeamColor, getTeamAbbreviation, getCenterIceLogo, rgbaToHex, } from "../utils";
/**
 ...
 */
export function createRink(params) {
    ...
    svgContent = svgContent.replace(/(<svg[^>]*?)(\s*viewBox="[^"]*")/, '$1 width="2400" height="1013"$2');
    ...
    svgContent = svgContent.replace(/(<svg[^>]*>)(\s*<g>)/, `$1${clipPathDef}$2`);
    // Apply clip-path to the main group
    svgContent = svgContent.replace(/(<svg[^>]*>\s*<defs>[\s\S]*?<\/defs>\s*)(<g>)/, '$1<g clip-path="url(#rinkClip)">');
    // Replace the elements with team-specific values
    // Replace left abbreviation
    svgContent = svgContent.replace(/(<text[^>]*id="abvLeft"[^>]*>)[^<]*(<\/text>)/, `$1${leftAbbrev}$2`);
    // Replace right abbreviation
    svgContent = svgContent.replace(/(<text[^>]*id="abvRight"[^>]*>)[^<]*(<\/text>)/, `$1${rightAbbrev}$2`);
    ...
    svgContent = svgContent.replace(/(<image[^>]*id="centerImage"[^>]*(?:xlink:)?href=")[^"]*(")/, `$1${logoData}$2`);
    // Add fill color to the style attribute for left team text
    svgContent = svgContent.replace(/(<text[^>]*id="abvLeft"[^>]*style="[^"]*)(")/, `$1fill: ${leftColor};$2`);
    // Add fill color to the style attribute for right team text
    svgContent = svgContent.replace(/(<text[^>]*id="abvRight"[^>]*style="[^"]*)(")/, `$1fill: ${rightColor};$2`);
    return svgContent;
}
/**
 ...
 */
export function createRinkToFile(params, outputPath) {
