import { renderTrackingFrame } from "./renderTrackingFrame";
 ...
 */
export async function createTrackingGif(rinkParams, frames, outputPath, options = {}) {
    const { highlightPlayerId, showNumbers = true, playerRadius = 38, puckRadius = 12, width = 2400, height = 1013, quality = 10, repeat = 0, frameSkip = 1, } = options;
    if (frames.length === 0) {
    ...
    for (let i = 0; i < selectedFrames.length; i++) {
        ...
        const svgContent = renderTrackingFrame(rinkParams, selectedFrames[i], {
            highlightPlayerId,
            showNumbers,
            playerRadius,
            puckRadius,
    ...
    const compressedPath = path.join(path.dirname(outputPath), path.basename(tempPath));
 ...
 */
export async function createTrackingGifFromRange(rinkParams, frames, outputPath, startIndex, endIndex, options = {}) {
