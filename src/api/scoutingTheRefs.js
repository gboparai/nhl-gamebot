 ...
 */
export function calculateTotalGames(seasonGames, careerGames) {
 ...
 */
export function extractOfficialData($, row, includesPenalties = false) {
    const officials = [];
 ...
 */
export function findRelevantPost(posts) {
    return (posts.find((post) => (post.categories.includes(REFS_CATEGORY_ID) && isToday(post.date)) ||
        (isToday(post.date) &&
            post.title.rendered.includes("NHL Referees and Linesmen"))) || null);
}
/**
 ...
 */
export async function fetchGameDetails(prefTeamFullName) {
    ...
    try {
        ...
        logger.error("Error fetching game details:", error instanceof Error ? error.message : "Unknown error");
        return emptyGameDetails;
    }
}
