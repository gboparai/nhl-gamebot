 ...
 */
export function convertTeamName(teamName) {
 ...
 */
export function extractLastUpdate($) {
 ...
 */
export function extractPlayerNames($, position) {
    return $(`#${position}`)
        .parent()
        .parent()
        .find("img")
 ...
 */
export function buildTeamUrl(teamName) {
 ...
 */
export async function fetchTeamPage(url) {
    try {
        ...
        throw new Error(`Failed to fetch team page: ${error instanceof Error ? error.message : 'Unknown error'}`);
    }
}
/**
 ...
 */
export async function dailyfaceoffLines(teamName) {
    const emptyLines = {
        confirmed: false,
        forwards: [],
        defense: [],
    ...
    try {
        ...
        logger.error("Error fetching lines data:", error instanceof Error ? error.message : "Unknown error");
        return emptyLines;
    }
}
