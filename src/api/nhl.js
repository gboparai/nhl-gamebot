 ...
 */
export async function fetchGameLanding(gameID) {
    try {
 ...
 */
export async function fetchNHLScores(date) {
    try {
 ...
 * @throws If there is an error while fetching the data.
 */
export async function fetchGameCenterRightRail(gameID) {
    try {
        const response = await axios.get(`${BASE_URL}/gamecenter/${gameID}/right-rail`);
        return response.data;
    }
    catch (error) {
