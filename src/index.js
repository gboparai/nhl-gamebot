...
import { fetchGameLanding, fetchBoxscore, fetchPlayByPlay, fetchNHLScores, fetchGameCenterRightRail, fetchTeamSummaries, } from "./api/nhl";
import { fetchLiveAdvancedGameStats, fetchTeamAverageAdvancedStats, } from "./api/hockeychart";
import { fetchGameDetails } from "./api/scoutingTheRefs";
...
import { convertUTCToLocalTime, getCurrentDateLocalTime, goalEmojis, thumbsDownEmojis, starEmojis, groupedList, getLastName, sleep, formatPeriodLabel, transformGameLandingToLineScores, setDateOverride } from "./utils";
...
(function (GameStates) {
    ...
    GameStates["PREGAME"] = "PREGAME";
    GameStates["INGAME"] = "INGAME";
    GameStates["POSTGAME"] = "POSTGAME";
    GameStates["POSTGAMETHREESTARS"] = "POSTGAMETHREESTARS";
    GameStates["POSTGAMEVID"] = "POSTGAMEVID";
...
let prefTeam;
let oppTeam;
let hasSentIntermission = false;
let sentEvents = [];
let goalPosts = [];
// Setter functions for testing
function setCurrentState(state) { currentState = state; }
function setCurrentGame(game) { currentGame = game; }
function setPrefTeam(team) { prefTeam = team; }
function setOppTeam(team) { oppTeam = team; }
function setHasSentIntermission(sent) { hasSentIntermission = sent; }
function setSentEvents(events) { sentEvents = events; }
function setGoalPosts(posts) { goalPosts = posts; }
async function generateLiveAdvancedStatsGraphic(options) {
    ...
    try {
        ...
        logger.warn(`[${new Date().toISOString()}] Live advanced stats endpoint unavailable, skipping advanced stats reply graphic:`, error);
        return false;
    }
}
async function generatePregameMatchupAdvancedStatsGraphic(options) {
    const { season, homeTeamAbbrev, awayTeamAbbrev, homeTeamName, awayTeamName, title, outputPath, gameType, } = options;
    let gameTypeStr = "regular";
    if (gameType === 1)
        gameTypeStr = "preseason";
    else if (gameType === 3)
        gameTypeStr = "playoff";
    try {
        ...
        logger.warn(`[${new Date().toISOString()}] Pregame team-average advanced stats endpoint unavailable, skipping pregame advanced stats reply graphic:`, error);
        return false;
    }
}
/**
 ...
 */
async function checkForHighlights() {
    if (!currentGame) {
            ...
            if (goal) {
                ...
                if (goal.details && goal.pptReplayUrl && !goalPost.gifSent) {
                    try {
                        ...
                        await send(edgeGoalText, currentGame, [gifPath], true, // extended = true (don't send to Twitter)
                        goalPost.blueskyPost, // Bluesky reply to original post
                        goalPost.threadsPost, // Threads reply to original post
                        goalPost.telegramPost);
                ...
                if (goal.details?.highlightClipSharingUrl && goal.details?.highlightClipSharingUrl !== 'https://nhl.com/video/' && !goalPost.videoSent) {
                    ...
                    await send(highlightText, currentGame, undefined, true, // extended = true (don't send to Twitter)
                    goalPost.blueskyPost, // Bluesky reply to original post
                    goalPost.threadsPost, // Threads reply to original post
                    goalPost.telegramPost);
...
const handleWaitingState = async () => {
    ...
    try {
        ...
        const nhlScores = await fetchNHLScores(getCurrentDateLocalTime(config.app.script.timeZone));
        currentGame = nhlScores.games.find((game) => game.awayTeam.abbrev === config.app.script.team ||
...
const handlePregameState = async () => {
    ...
    if (currentGame !== undefined) {
        try {
            ...
            const homeTeamSummary = teamSummaries.data.find((team) => team.teamId === currentGame.homeTeam.id);
            const awayTeamSummary = teamSummaries.data.find((team) => team.teamId === currentGame.awayTeam.id);
            ...
            if (refereeDetails?.confirmed === false && msUntilStart > 30 * 60 * 1000) {
                ...
                await send(`Tune in tonight when the ${prefTeam?.name.default} take on the ${oppTeam?.name.default} at ${currentGame.venue.default}.\n\n🕢 ${formattedTime12Hr}\n📺 ${currentGame.tvBroadcasts.map((broadcast) => broadcast.network).join(", ")}`, currentGame, [`./temp/preGame.png`]);
                ...
                const gamePost = await send(`🔥 Get ready for an epic showdown! Tonight, it's the ${prefTeam?.name.default} going head-to-head with the ${oppTeam?.name.default} at ${currentGame.venue.default}. You won’t want to miss a second of the action! `, currentGame, [`./temp/game.png`]);
                try {
                    const pregameAdvancedStatsPath = `./temp/pregame-advanced-stats.png`;
                    const advancedStatsGraphicGenerated = await generatePregameMatchupAdvancedStatsGraphic({
                    ...
                    if (advancedStatsGraphicGenerated) {
                        await send(`Pregame matchup advanced stats`, currentGame, [pregameAdvancedStatsPath], true, gamePost.blueskyPost, gamePost.threadsPost, gamePost.telegramPost, gamePost.twitterPost);
                    }
                }
                catch (advancedStatsError) {
                    logger.error(`[${new Date().toISOString()}] Error generating/sending pregame advanced stats reply:`, advancedStatsError);
                }
                ...
                if (dfLines.confirmed) {
                    ...
                    await send(`Projected lines for the ${prefTeam?.name.default} (via @DailyFaceoff)\n\n${groupedList(dfLines.forwards.map((player) => getLastName(player)), 3)}\n${groupedList(dfLines.defense.map((player) => getLastName(player)), 2)}\n${groupedList(dfLines.goalies.map((player) => getLastName(player)), 1)}`, currentGame);
                }
                ...
                if (dfLinesOpps.confirmed) {
                    ...
                    await send(`Projected lines for the ${oppTeam?.name.default} (via @DailyFaceoff)\n\n${groupedList(dfLinesOpps.forwards.map((player) => getLastName(player)), 3)}\n${groupedList(dfLinesOpps.defense.map((player) => getLastName(player)), 2)}\n${groupedList(dfLinesOpps.goalies.map((player) => getLastName(player)), 1)}`, currentGame);
                }
                else {
                ...
                if (refereeDetails?.confirmed) {
                    ...
                    const linesmens = refereeDetails.linesmens
                        ...
                        .join("\n");
                    await send(`The officials (via @ScoutingTheRefs)\n\n${referees}\n${linesmens}`, currentGame);
                }
                else {
    ...
    try {
        ...
        if (playByPlay.clock.inIntermission && hasSentIntermission) {
            ...
            if (playByPlay.plays.length > 0) {
                ...
                const relevantPlays = playByPlay.plays.filter((play) => {
                    ...
                    return (play.typeDescKey === "goal" ||
                        ...
                        play.typeDescKey === "game-end") && !sentEvents.includes(play.eventId);
                });
                    ...
                    for (const play of sortedPlays) {
                        ...
                        try {
                            if (play.typeDescKey === "goal") {
                                ...
                                const scoringTeam = play.details?.eventOwnerTeamId === currentGame?.awayTeam.id
                                    ? currentGame?.awayTeam
                                    : currentGame?.homeTeam;
                                const scoringTeamsScore = play.details?.eventOwnerTeamId === currentGame?.awayTeam.id
                                ...
                                if (play.details?.assist1PlayerId) {
                                    const assist1Player = playByPlay?.rosterSpots.find((player) => player.playerId === play.details?.assist1PlayerId);
                                    if (assist1Player) {
                                ...
                                if (play.details?.assist2PlayerId) {
                                    const assist2Player = playByPlay?.rosterSpots.find((player) => player.playerId === play.details?.assist2PlayerId);
                                    if (assist2Player) {
                                ...
                                const goalie = playByPlay?.rosterSpots.find((player) => player.playerId === play.details?.goalieInNetId);
                            ...
                            \n${scoringPlayer?.firstName.default} ${scoringPlayer?.lastName.default} (${play.details?.scoringPlayerTotal}) scores with ${play.timeRemaining} left in the ${formatPeriodLabel(play.periodDescriptor.number, playByPlay.gameType)} period.${assistsText}
                                ...
                                const drawnTeam = play.details?.eventOwnerTeamId !== currentGame?.awayTeam.id
                                    ? currentGame?.awayTeam
                                    : currentGame?.homeTeam;
                                const penaltyPlayer = playByPlay?.rosterSpots.find((player) => player.playerId === play.details?.committedByPlayerId || play.details?.servedByPlayerId);
                                // Get the player who drew the penalty (if any)
                                const drawnByPlayer = play.details?.drawnByPlayerId
                                    ? playByPlay?.rosterSpots.find((player) => player.playerId === play.details?.drawnByPlayerId)
                                    : null;
                                ...
                                try {
                                    ...
                                    if (boxscoreNow.homeTeam.score !== undefined && boxscoreNow.awayTeam.score !== undefined) {
                                        await send(`It's time for the ${formatPeriodLabel(play.periodDescriptor.number, boxscoreNow.gameType)} period at ${currentGame.venue.default}. Let's go ${prefTeam?.name.default}!\n\n${scoreText}`, currentGame);
                                    }
                                    else {
                                    ...
                                    await send(`It's time for the ${formatPeriodLabel(play.periodDescriptor.number, playByPlay.gameType)} period at ${currentGame.venue.default}. Let's go ${prefTeam?.name.default}!`, currentGame);
                                }
                            }
                                ...
                                if (!gameEndEvent) {
                                    ...
                                    try {
                                        ...
                                        const intermissionPost = await send(`It's end of the ${formatPeriodLabel(play.periodDescriptor.number, playByPlay.gameType)} period at ${currentGame.venue.default}\n\n${currentGame?.homeTeam.name.default}: ${boxscore.homeTeam.score}\n${currentGame?.awayTeam.name.default}: ${boxscore.awayTeam.score}`, currentGame, [`./temp/intermission.png`]);
                                        try {
                                            ...
                                            if (advancedStatsGraphicGenerated) {
                                                await send(`Advanced stats through ${formatPeriodLabel(play.periodDescriptor.number, playByPlay.gameType)} period`, currentGame, [intermissionAdvancedStatsPath], true, intermissionPost.blueskyPost, intermissionPost.threadsPost, intermissionPost.telegramPost, intermissionPost.twitterPost);
                                            }
                                        }
                                        catch (advancedStatsError) {
                                        ...
                                        try {
                                            ...
                                            await createShotChart(String(currentGame.id), './temp/intermission-shot-chart-heatmap.png', { includeHeatmap: true, includeShots: false, normalizeByTeam: true });
                                            logger.info(`[${new Date().toISOString()}] Creating shots-only chart...`);
                                            await createShotChart(String(currentGame.id), './temp/intermission-shot-chart-shots.png', { includeHeatmap: false, includeShots: true, normalizeByTeam: true });
                                            // Send both charts in one post as reply
                                            logger.info(`[${new Date().toISOString()}] Sending both shot charts as reply...`);
                                            await send(`Shot charts through ${formatPeriodLabel(play.periodDescriptor.number, playByPlay.gameType)} period`, currentGame, [
                                                './temp/intermission-shot-chart-heatmap.png',
                                                './temp/intermission-shot-chart-shots.png'
                                            ], true, intermissionPost.blueskyPost, intermissionPost.threadsPost, intermissionPost.telegramPost, intermissionPost.twitterPost);
...
const handlePostGameState = async () => {
    ...
    try {
        ...
        const postGamePost = await send(`The ${winningTeam} defeat the ${losingTeam} at ${currentGame.venue.default}!\n\n${currentGame?.homeTeam.name.default}: ${boxscore.homeTeam.score}\n${currentGame?.awayTeam.name.default}: ${boxscore.awayTeam.score}`, currentGame, [`./temp/postGame.png`]);
        try {
            ...
            if (advancedStatsGraphicGenerated) {
                await send(`Final advanced stats for tonight's game`, currentGame, [postgameAdvancedStatsPath], true, postGamePost.blueskyPost, postGamePost.threadsPost, postGamePost.telegramPost, postGamePost.twitterPost);
            }
        }
        catch (advancedStatsError) {
        ...
        try {
            ...
            await createShotChart(String(currentGame.id), './temp/postgame-shot-chart-full.png', { includeHeatmap: true, includeShots: false, normalizeByTeam: true });
            logger.info(`[${new Date().toISOString()}] Creating shots-only chart...`);
            await createShotChart(String(currentGame.id), './temp/postgame-shot-chart-shots.png', { includeHeatmap: false, includeShots: true, normalizeByTeam: true });
            // Send both charts in one post as reply
            logger.info(`[${new Date().toISOString()}] Sending both shot charts as reply...`);
            await send(`Final shot charts for tonight's game`, currentGame, [
                './temp/postgame-shot-chart-full.png',
                './temp/postgame-shot-chart-shots.png'
            ], true, postGamePost.blueskyPost, postGamePost.threadsPost, postGamePost.telegramPost, postGamePost.twitterPost);
            logger.info(`[${new Date().toISOString()}] Shot charts reply sent`);
        }
        catch (shotChartError) {
...
const handlePostGameThreeStarsState = async () => {
    ...
    try {
        const gameLanding = await fetchGameLanding(String(currentGame.id));
        if (gameLanding?.summary.threeStars !== undefined &&
            ...
            const threeStars = gameLanding.summary.threeStars
                ...
                .join("\n");
            await send(`Tonight's Three Stars\n\n${threeStars}`, currentGame);
...
const handlePostGameVideoState = async () => {
    ...
    try {
        ...
        if (bothVideosAvailable) {
            ...
            await send(`Check out the condensed game of tonight's match between the ${currentGame?.homeTeam.name.default} and the ${currentGame?.awayTeam.name.default}:\n\n${condensedUrl}`, currentGame);
            // Send recap video
            const recapUrl = `https://www.nhl.com/video/topic/game-recaps/${awayTeamAbbrev}-at-${homeTeamAbbrev}-recap-${recapVideo}`;
            await send(`Check out the recap of tonight's match between the ${currentGame?.homeTeam.name.default} and the ${currentGame?.awayTeam.name.default}:\n\n${recapUrl}`, currentGame);
...
export { GameStates, currentState, currentGame, prefTeam, oppTeam, hasSentIntermission, sentEvents, goalPosts, setCurrentState, setCurrentGame, setPrefTeam, setOppTeam, setHasSentIntermission, setSentEvents, setGoalPosts, checkForHighlights, handleWaitingState, handlePregameState, handleInGameState, handlePostGameState, handlePostGameThreeStarsState, handlePostGameVideoState, handleEndGameState, main };
