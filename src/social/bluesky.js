 ...
 */
export async function sendBlueskyPost(text, game, media, retries = 3, replyTo) {
    const operation = async () => {
        ...
        if (media && media.length > 0) {
            const uploadedMedia = await Promise.all(media.map(async (mediaPath) => {
                try {
 ...
 */
export async function uploadBlueskyMedia(mediaPath) {
    try {
        ...
        logger.error("Error uploading media to Bluesky:", error.message);
        throw error;
    }
}
/**
 ...
 */
function getBlueskyHashtags(game) {
